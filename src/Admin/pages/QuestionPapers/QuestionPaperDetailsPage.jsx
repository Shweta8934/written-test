...
const QuestionPaperDetailsPage = () => {
  ...
  if (loading) {
    return (
      <div className="qpd-page">
        <div className="qpd-loader">
          <FaSpinner className="spin" /> Loading paper details...
        </div>
      </div>
    );
  }
  ...
  if (!paper) {
    return (
      <div className="qpd-page">
        <div className="qpd-empty">
          <p>⚠️ Paper not found.</p>
          <button className="qpd-btn primary-btn" onClick={() => navigate(-1)}>
            <FaArrowLeft /> Go Back
          </button>
        </div>
      </div>
      ...
      <div className="qpd-container">
        <header className="qpd-header">
          <div className="header-info">
            <h1 className="qpd-title">{paper.paperName}</h1>
            <p className="qpd-meta">
              <span>Duration:{paper.duration} mins</span>
              <span>Questions: {paper.numOfQuestions}</span>
            </p>
          </div>
          <button
            className="qpd-btn secondary-btn"
            onClick={() => navigate(-1)}
          >
            <FaArrowLeft /> Back
          </button>
        ...
        <button
          className="qpd-btn primary-btn add-question-btn"
        ...
        {showAllQuestions && (
          <section className="qpd-all-questions-section">
            <h2 className="section-title">Select Questions to Add</h2>
            ...
            {allQuestions.length > 0 && (
              <button
                className="qpd-btn primary-btn add-selected-btn"
                onClick={handleAddQuestions}
                disabled={adding}
              >
                {adding ? (
        ...
        <section className="qpd-questions-section">
          <h2 className="section-title">Questions in Paper</h2>
          <div className="questions-grid">
            {paper.questions.length === 0 ? (
              <p className="empty-state">
                No questions in this paper yet. Add some to get started!
              </p>
            ) : (
              paper.questions.map((q, index) => (
                <div key={q.id} className="qpd-question-card">
                  ...
                  <div className="card-meta">
                    ...
                    <span className="card-meta-item">
                      Topic: <strong>{q.questionTopic}</strong>
                    </span>
                    <span className="card-meta-item">
                      Level: <strong>{q.questionLevel}</strong>
                    </span>
                  </div>
                  ...
                  <div className="qpd-actions">
                    <button
                      className="qpd-action-btn edit-btn"
                      onClick={() => openEditModal(q)}
                    >
                      <FaEdit /> Edit
                    </button>
                    <button
                      className="qpd-action-btn delete-btn"
                      onClick={() => handleDeleteQuestion(q.id)}
                    >
                      <FaTrashAlt /> Delete
                    </button>
          ...
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <form onSubmit={handleSubmit}>
                ...
                {/* ✅ Only show options if questionType is NOT coding or one line */}
                {formData.questionType !== "coding" &&
                  formData.questionType !== "one line answer" && (
                    <div className="form-group">
                ...
                <div className="modal-buttons">
                  <button type="submit" className="qpd-btn success-btn">
                    <FaSave /> Save Changes
                  </button>
                </div>
              </form>
